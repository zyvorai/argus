...
Full steps (Helm, BYO OIDC, local auth, troubleshooting): [Enterprise SSO / OIDC](../../enterprise-sso.md).

If Mission Control login fails, hit `GET /health`, confirm `argus serve` is up, and re-check env from [.env.example](../../../../.env.example).
